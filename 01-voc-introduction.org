...
#+header: :file images/intro-relationship-ladder.png
#+begin_src R :results output file graphics  :exports results
  # Relationship ladder for IWA journals
  library(ggplot2)

  ladder <- data.frame(label = c("Public", "Community", "Household",
                                 "User", "Consumer", "Customer"),
                       expectation = 1:6,
                       obligation = 1:6,
                       entries = c(2425, 1282, 555, 251, 208, 194),
                       dy = c(0, 0, 2, 2, 3, 3))

  ggplot(ladder) +
    aes(x = expectation, y = obligation, label = label) +
    geom_point(aes(size = entries), colour = "grey", show.legend = FALSE) +
    scale_size_area(max_size = 60) +
    geom_text(color = "gray20", vjust = ladder$dy) +
...
The fact that customers cannot choose a different water utility when they are unhappy with the service they receive places greater importance on water utilities understanding the customers. The power of buyers in the utility market has a political dimension and utilities are motivated to be customer-centric through political pressure and regulation [cite:@prevos_2017_cust].

Another significant difference between commercial organisations and water utilities is that commercial companies seek to maximise sales, while water utilities want to restrain consumption to preserve natural resources. While the commercial world focuses on marketing, water utilities practice 'demarketing' [cite:@prevos_2017_cust; @kotler_1971_demar].
...
* EDITED Phenomenology of the Customer Experience
:NOTES:
- [ ] Latent variables
- [ ] neuromarketing
:END:
...
#+begin_src dot :file images/intro-voice-of-the-customer.png
  digraph voc {
      dpi=300;
      rankdir=TB;
      nodesep=0.6;
...
* EDITED Supplementary Material and Licensing
